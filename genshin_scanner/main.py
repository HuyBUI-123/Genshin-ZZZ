...
class ScannerApp(ctk.CTk):
    ...
    def _build_payload(self) -> list[dict]:
        """The artifact list in the web app's create-schema shape."""
        out = []
    ...
    def _export(self):
        ...
        self.export_status.configure(
            text=f"Saved {len(out)} artifact(s) → {path}{self._unscored_note()}"
        )
    ...
    def _copy_json(self):
        ...
        self.export_status.configure(
            text=f"Copied {len(out)} artifact(s) to clipboard{self._unscored_note()}"
        )
