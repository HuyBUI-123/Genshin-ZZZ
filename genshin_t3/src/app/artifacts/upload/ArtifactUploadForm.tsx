...
function validateEntry(raw: unknown, index: number): ValidatedEntry {
  ...
  const unact =
    typeof r.unactivatedSubstat === "string" ? (r.unactivatedSubstat as string) : null;
  if (n === 3 && !unact) {
    errors.push("3-substat artifact needs an unactivatedSubstat");
  }
...
export default function ArtifactUploadForm() {
  ...
  const createMany = api.artifact.createMany.useMutation({
    onSuccess: (res) => {
      setNotification({
        type: "success",
        message: `Uploaded ${res.count} artifact(s) successfully!`,
      });
      setEntries([]);
      setFileName(null);
      router.refresh();
        ...
        <>
                  ...
                  {valid.map((e) => (
                    <tr key={e.index} className="border-t border-slate-700/50 text-gray-200">
                      ...
                      <td className="px-3 py-2">
                        {e.data!.substats.join(", ")}
                        {e.data!.unactivatedSubstat
                          ? ` (+${e.data!.unactivatedSubstat})`
                          : ""}
                      </td>
          ...
          >
            {createMany.isPending
              ? "Uploading..."
              : `Upload ${valid.length} artifact(s)`}
          </button>
        </>
      )}
    </div>
